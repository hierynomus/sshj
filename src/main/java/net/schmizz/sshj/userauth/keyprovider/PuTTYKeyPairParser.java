...
/**
 * Parses PuTTY private key files ({@code .ppk}, versions 1-3).
 *
 ...
 * </pre>
 */
class PuTTYKeyPairParser implements KeyPairParser {
    ...
    public KeyPair parseKeyPair(final Resource<?> resource, final PasswordFinder pwdf) throws IOException {
        this.loadKeyMaterial(resource, pwdf);
    ...
    /**
     * Key type as declared in the {@code PuTTY-User-Key-File-N} header. Only meaningful once
     * {@link #parseKeyPair} has run.
     */
    KeyType getType() {
    ...
    int getKeyFileVersion() {
        return keyFileVersion;
    }

    private void loadKeyMaterial(final Resource<?> resource, final PasswordFinder pwdf) throws IOException {
