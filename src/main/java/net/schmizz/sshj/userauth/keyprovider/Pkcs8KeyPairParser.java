...
/**
 * Parses PEM-encoded PKCS8 and PKCS1 private keys, with or without password-based encryption.
 * Used by {@link PKCS8KeyFile} and {@link OpenSSHKeyFile}.
 */
class Pkcs8KeyPairParser implements KeyPairParser {
    ...
    @Override
    public KeyPair parseKeyPair(final Resource<?> resource, final PasswordFinder pwdf) throws IOException {
        ...
        try (BufferedReader bufferedReader = new BufferedReader(resource.getReader())) {
            ...
            return readKeyPair(pemKey, resource, pwdf);
        }
    }

    private KeyPair readKeyPair(final PEMKey pemKey, final Resource<?> resource, final PasswordFinder pwdf) throws IOException {
        ...
        if (PEMKeyType.DSA == pemKeyType) {
            ...
            keyPair = readEncryptedPkcs8KeyPair(pemKeyBody, resource, pwdf);
    ...
    private KeyPair readEncryptedPkcs8KeyPair(final byte[] pemKeyBody, final Resource<?> resource, final PasswordFinder pwdf) throws IOException {
        if (pwdf == null) {
