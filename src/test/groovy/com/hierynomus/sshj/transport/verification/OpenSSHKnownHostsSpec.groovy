...
class OpenSSHKnownHostsSpec extends Specification {
    private static final Base64Decoder BASE_64_DECODER = Base64Provider.decoder
...
""")
        def pk = new Buffer.PlainBuffer(BASE_64_DECODER.decode("AAAAE2VjZHNhLXNoYTItbmlzdHAyNTYAAAAIbmlzdHAyNTYAAABBBLTjA7hduYGmvV9smEEsIdGLdghSPD7kL8QarIIOkeXmBh+LTtT/T1K+Ot/rmXCZsP8hoUXxbvN+Tks440Ci0ck=")).readPublicKey()
        when:
...
"schmizz.net,69.163.155.180 ssh-rsa  extra   space"
    ...
    def "should match any host name from multi-host line"() {
        ...
        def pk = new Buffer.PlainBuffer(BASE_64_DECODER.decode("")).readPublicKey()
    ...
    def "should forgive redundant spaces like OpenSSH does"() {
        ...
        def f = knownHosts("""
          ...
          """.stripMargin())
        def pk = new Buffer.PlainBuffer(BASE_64_DECODER.decode(key)).readPublicKey()
    ...
    def "should not throw errors while parsing corrupted records"() {
        ...
        then:
        knownhosts.verify("good-host", 22, new Buffer.PlainBuffer(BASE_64_DECODER.decode(key)).readPublicKey())
    }
