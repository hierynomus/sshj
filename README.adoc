...
* SSH Extension Negotiation (https://datatracker.ietf.org/doc/html/rfc8308[RFC 8308]): advertises `ext-info-c` and uses the server's `server-sig-algs` extension to prefer public-key signature algorithms the server actually accepts during publickey authentication
